...
public class SeatService {
    ...
    public List<SeatAvailabilityResponse> listSeatAvailability(String showtimeId) {
        ...
        List<SeatReservation> reservations = seatReservationService.listShowtimeReservations(showtimeId);
        Map<String, SeatAvailabilityStatus> statusBySeatId = new HashMap<>();

        for (SeatReservation reservation : reservations) {
            SeatAvailabilityStatus seatStatus = reservation.getState() == SeatReservationState.BOOKED
                    ? SeatAvailabilityStatus.BOOKED
                    : SeatAvailabilityStatus.RESERVED;
            SeatAvailabilityStatus existing = statusBySeatId.get(reservation.getSeatId());
            if (existing == SeatAvailabilityStatus.BOOKED) {
                continue;
            }
            statusBySeatId.put(reservation.getSeatId(), seatStatus);
        }
        ...
        return seats.stream().map(seat -> {
            ...
            if (seat.getStatus() == SeatStatus.OUT_OF_SERVICE) {
                ...
                status = statusBySeatId.getOrDefault(seat.getId(), SeatAvailabilityStatus.AVAILABLE);
            }
